...
function RunSimulationA(NodePositions, NodePropagationDelays, NodeDeliveryProbabilities, NodeLabels, NodeAddresses, SpeedOfSound, IterationCount)
    ...
    RunDuration = 0.5*60.0*60.0; % seconds (30 minutes)
    ...
    SensorTransmitPeriodTime = 0.5*60.0*60.0; % seconds (30 minutes)
    SensorTransmitPeriodStepCount = floor(SensorTransmitPeriodTime / StepPeriod); 

    SlotCount = 10;
    NodeCount = length(NodeDeliveryProbabilities(:,1));
    NodeTransmitTimes = [0:SlotCount*NodeCount-1] * (SensorTransmitPeriodTime / (SlotCount*NodeCount)); % seconds
    NodeTransmitStepCounts = floor(NodeTransmitTimes ./ StepPeriod);
    
    ModemBitRate = 100.0; % Arbitrary data rate of 100bps 

    % Packet ready probability (real, [0,1]): the probability that a given source has a packet ready to be transmitted at any given time slot.
    TransmitProbability = 0.2;
    
    sourceStatus = zeros(1,NodeCount);
    ...
    sourceBackoff = zeros(1,NodeCount);
    % Maximum backoff (positive integer): value that a backlogged source must wait before a new transmission attempt.
    maxBackoff = 10; 

    % Initialization of counters for successes and collisions
    SuccessCount = 0;
    CollisionCount = 0;
    ...
    for StepIdx = 1:RunStepCount
        ...
        for Slot = 1:SlotCount*NodeCount
            NodeIdx = mod(Slot,NodeCount);
            if NodeIdx == 0
                NodeIdx = NodeCount;
            end
            ...
            if NodeIdx > 1 % Sensor Nodes Only
                ...
                if StepIdx-1 == NodeTransmitStepCounts(Slot)
                    % Transmit New Sensor Data
                    
                    for eachSource1 = 2:length(sourceStatus)
                        if sourceStatus(1,eachSource1) == 0 && rand(1) <= TransmitProbability % new packet
                    ...
                    if sum(sourceStatus == 1) == 1
                        SourceAddress = NodeAddresses(Node);
                        ...
                        % Put the Transmit packet in the queue for the simulator core to handle.
                        TransmittedPackets{Node}{length(TransmittedPackets{Node})+1} = TxPacket;
                        TransmittedPacketsEmpty = false;

                        % Store information for handling Acks/Retries
                        NodeAwaitingAcks(Node) = 1;
                        NodePacketsToResend{Node} = TxPacket;
                        NodeRetriesCount(Node) = 0;
                    end
    ...
    SuccessCount
    CollisionCount
    throughput = SuccessCount / (SlotCount*NodeCount);
    pcktCollisionProb = CollisionCount / (SlotCount*NodeCount);
    fprintf('\nThroughput (S): %.3f,\nCollision probability (P_c): %.3f.\n',throughput,pcktCollisionProb);
...
function DisplayNodeStateLogs(NodeStateLogs, NodeLabels, StepPeriod)
    ...
    for NodeIdx = 1:NodeCount
        ...
        stem(t(NodeStateLogs(NodeIdx,:) == 1), NodeStateLogs(NodeIdx, NodeStateLogs(NodeIdx,:) == 1), 'Marker','none');
        hold on;        
        stem(t(NodeStateLogs(NodeIdx,:) == 3), NodeStateLogs(NodeIdx, NodeStateLogs(NodeIdx,:) == 3), 'Marker','none');
        stem(t(NodeStateLogs(NodeIdx,:) == 2), NodeStateLogs(NodeIdx, NodeStateLogs(NodeIdx,:) == 2), 'LineWidth',1.5, 'Marker','x', 'MarkerSize',10);
        hold off;
        ...
        for ToIdx = 1:NodeCount
            if ~isempty(Channels{FromIdx, ToIdx})
                ...
                for PacketIdx = 1:length(Channels{FromIdx, ToIdx})
                    Pack = Channels{FromIdx, ToIdx}{PacketIdx};
                    OuterDiameter = (CurrentTime - Pack.TransmitStartTime) * SpeedOfSound;
                    InnerDiameter =  (CurrentTime - Pack.TransmitStartTime - Pack.TransmitDuration) * SpeedOfSound;
                    if InnerDiameter < 0
...
function Pack = Packet(SourceAddress, DestinationAddress, PayloadBytes, TransmitStartTime, TransmitDuration)
